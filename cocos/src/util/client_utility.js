...
cutil.give_card = function(delta_card, to_user_id, callback) {
	var info_dict = eval('(' + cc.sys.localStorage.getItem("INFO_JSON") + ')');
	var bind_xhr = cc.loader.getXMLHttpRequest();
	bind_xhr.open("POST", switches.PHP_SERVER_URL + "/api/zhuanka", true);
	bind_xhr.onreadystatechange = function() {
		cutil.unlock_ui();
		if(bind_xhr.readyState === 4 && bind_xhr.status === 200) {
			cc.log(bind_xhr.responseText);
			if (bind_xhr.responseText[0] == "{") {
				var give_card_dict = JSON.parse(bind_xhr.responseText);
				if (give_card_dict["errcode"] == 0) {
					cc.log("Give Card Success!");
					if(callback) {
						callback(give_card_dict);
					}
					if(h1global.curUIMgr.gamehall_ui && h1global.curUIMgr.gamehall_ui.is_show){
						h1global.curUIMgr.gamehall_ui.updateCharacterCard();
					}
					h1global.globalUIMgr.info_ui.show_by_info("转卡成功！");
				} else {
					cc.log("Get Give Card Error! The Error Code is " + give_card_dict["errmsg"].toString() + "!");
					h1global.globalUIMgr.info_ui.show_by_info("转卡失败！" + give_card_dict["errmsg"].toString(), cc.size(300, 200));
				}
			} else {
				cc.log("The Give Card is Fail!");
				h1global.globalUIMgr.info_ui.show_by_info("转卡失败！", cc.size(300, 200));
			}
		}
	};
	bind_xhr.setRequestHeader("Authorization", "Bearer " + info_dict["token"]);
	bind_xhr.send(cutil.postDataFormat({"delta_card" : delta_card, "to_user_id" : to_user_id}));
};
...
cutil.keyConvertFilters = ["GAME_ROOM_UI", "GAME_ROOM_BG"];
