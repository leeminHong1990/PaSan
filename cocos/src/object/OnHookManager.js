...
OnHookManager.prototype.update = function(interval){
	...
	if(this.is_PS_rec_time && this.is_PS_rec_time.length===3){
		this.is_PS_rec_time[1] = this.is_PS_rec_time[1]-interval;
		if (h1global.curUIMgr &&  h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow()) {
			h1global.curUIMgr.roomLayoutMgr.notifyObserver( "update_wait_time_left", this.is_PS_rec_time[1]-interval,this.is_PS_rec_time[2]);
			this.is_PS_rec_time = false;
		}
	}
};
...
OnHookManager.prototype.setPSWaitLeftTime = function(waitLeftTime,sumTime){
	// 单位是秒
	//在这里对爬三游戏场的时间进行操作
	if (h1global.curUIMgr &&  h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow()) {
		h1global.curUIMgr.roomLayoutMgr.notifyObserver( "update_wait_time_left", waitLeftTime,sumTime);
	}else{
		this.is_PS_rec_time = [true,waitLeftTime,sumTime];
	}
};