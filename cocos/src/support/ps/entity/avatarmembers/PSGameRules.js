...
/*-----------------------------------------------------------------------------------------
 interface
 -----------------------------------------------------------------------------------------*/
var PSGameRules = PSRoomOperationAdapter.extend({
	...
	compareCards: function (src, dest, converted, ignoreGreater) {
		if (!converted) {
			src = ps_rules.convert(src, false, false);
			dest = ps_rules.convert(dest, false, false);
		}

		function compare(infoA, infoB) {
			if (ps_rules.SEQS.indexOf(infoA[1]) >= 0) {
				if (infoB[2] > infoA[2] && infoB[3] - infoB[2] === infoA[3] - infoA[2]) {
					return infoB[2] - infoA[2];
				}
			} else {
				return ps_rules.compare_rank(infoB[2], infoA[2]);
			}
			return -1;
		}

		let hasFlower = this.curGameRoom.flower_mode === const_ps.MODE_HAS_FLOWER;

		let srcInfo = ps_rules.test_with_rule(src, true, true, true, hasFlower);
		if (!srcInfo[0]) {
			return -1;
		}
		let srcType = srcInfo[1];
		let func = ps_rules.COMPARE_TYPE_FUNC_MAP[srcType];
		...
		let types = ps_rules.getGreaterThan(srcType);
		if (types !== null) {
			for (var i = 0; i < types.length; i++) {
				if (!hasFlower && types[i] === ps_rules.TYPE_FLOWER) {
					continue;
				}
				let info = ps_rules.COMPARE_TYPE_FUNC_MAP[types[i]](dest);
				if (info[0]) {
					return true;
				}
			}
	...
	canDiscard: function (cards, serverSitNum) {
		...
		if (lastCards) {
			...
			// 可以出任意牌
			return ps_rules.test_with_rule(cards, false, false, false, hasFlower);
		}
	},
	...
	getGreaterThanCards: function (type, src, ignoreTail, ignoreGreater, targetCards) {
		...
		targetCards = targetCards || ps_rules.convert(this.curGameRoom.handTilesList[this.serverSitNum], false, false);
		let iter = ps_rules.ITER_TYPE_FUNC_MAP[type];
		let iterData = iter(targetCards, true, true, ignoreTail && ps_rules.IGNORE_TAIL_FUNC.indexOf(iter) >= 0);
		if (!src) {
			return iterData;
		}
		let hasFlower = this.curGameRoom.flower_mode === const_ps.MODE_HAS_FLOWER;
		...
		if (!srcInfo[0]) {
			...
			return ps_rules.emptyIter(targetCards, "data_error");
		}

		let srcPriority = ps_rules.getPriority(srcInfo[1]);
		let destPriority = ps_rules.getPriority(type);
		if (srcInfo[0]) {
			// Note:  排除不满足条件选项
			if (srcPriority > destPriority || (srcPriority === destPriority && srcInfo[1] !== type)) {
				return ps_rules.emptyIter(targetCards, "priority_ignore_" + type + "_" + srcInfo[1]);
			}
		}
	...
	hasGreaterThan: function () {
		...
		let srcInfo = ps_rules.test_with_rule(src, true, true, true, hasFlower);
		if (srcInfo[0]) {
			let types = ps_rules.getGreaterThan(srcInfo[1]);
			if (types === null) {
				types = [srcInfo[1]];
			} else {
	...
	getBestTip: function (selectCards) {
		selectCards = ps_rules.convert(selectCards, false, false);
		let lastCards = this.curGameRoom.getLastDiscard(this.serverSitNum);
		let lastType = null;
		let hasFlower = this.curGameRoom.flower_mode === const_ps.MODE_HAS_FLOWER;
		if (lastCards) {
			let lastInfo = ps_rules.test_with_rule(lastCards, false, false, true, hasFlower);
			if (lastInfo) {
				lastType = lastInfo[1];
			}
		}
		let best = null;
		for (var i = 0; i < ps_rules.ALL_TYPES.length; i++) {
			if (!hasFlower && ps_rules.ALL_TYPES[i] === ps_rules.TYPE_FLOWER) {
				continue;
			}
			let iter = this.getGreaterThanCards(ps_rules.ALL_TYPES[i], lastCards, true, false, selectCards);
			if (lastType === ps_rules.ALL_TYPES[i]) {
				if (iter.hasNext()) {
					return iter.next();
				}
			} else {
