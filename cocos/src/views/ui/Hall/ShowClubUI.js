var ShowClubUI = UIBase.extend({
    ...
    initUI:function(){
        ...
        var my_club_btn = this.show_club_panel.getChildByName("my_club_btn");
        if (Object.keys(club_list).length === 0) {
	        my_club_btn.setTouchEnabled(false);
	        my_club_btn.setBright(false);
        }
	    function my_club_btn_event(sender, eventType) {
		    if (eventType === ccui.Widget.TOUCH_ENDED) {
			    if(h1global.curUIMgr.clubmgr_ui && !h1global.curUIMgr.clubmgr_ui.is_show){
				    for (var i in club_list) {
					    if(club_list[i].club_id) {
						    h1global.curUIMgr.clubmgr_ui.show_by_info(club_list[i].club_id, true);
						    break;
					    }
				    }
			    }
	    ...
	    my_club_btn.addTouchEventListener(my_club_btn_event);
    },
});