...
var ApplyCloseUI = BasicDialogUI.extend({
	...
	initUI:function(){
        ...
        for(var i = 0 ; i < player.curGameRoom.player_num ; i++){
			...
			if(player.curGameRoom.gameType === const_val.PaSan && player.curGameRoom.playerInfoList[i]["identify"] == const_ps.GAME_VIEWER){
				player_info_panel.setVisible(false);
				continue;
			}
	...
	show_by_sitnum:function(serverSitNum){
		...
		if(player.curGameRoom.gameType === const_val.PaSan && player.curGameRoom.playerInfoList[serverSitNum]["identify"] == const_ps.GAME_VIEWER){
			return;
		}
		this.show(function(){
