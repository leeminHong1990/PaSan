var UICommonWidget = {
	...
	*/
	...
	add_btn_cd: function (parent_btn, resStencil, set_num, num_size, num_color) {
		...
		progressCooling.ignoreAnchorPointForPosition(true);
	...
	set_btn_cd: function (parent_btn, wait_time, time_left, show_num) {
		...
		if (time_left <= 0) {
			...
			progressCooling.stopAllActions();
			if (show_num) {
	 ...
	 * @return null;
	 */
	set_ps_btn_cd: function (parent_btn, wait_time, time_left) {
		if (!cc.sys.isObjectValid(parent_btn) || !parent_btn.getChildByName("progressCooling")) {
			return;
		}
		if (time_left <= 0) {
			...
			parent_btn.runAction(cc.sequence(
				cc.delayTime(time_left > 5 ? (time_left - 5) : 0),
				cc.callFunc(()=>{
					if(parent_btn && parent_btn.isVisible()){
						parent_btn.is_time_out = true;
						parent_btn.runAction(cc.repeat(cc.sequence(cc.callFunc(function () {
							if(parent_btn.is_time_out){
								cc.audioEngine.playEffect("res/sound/voice/poker_ps/time_out.mp3");
							}
						}), cc.delayTime(1.0)), time_left < 5 ? time_left : 5));
					}
				})
			));
			// parent_btn.runAction(cc.repeat(cc.sequence(cc.callFunc(function () {
			...
			// setTimeout(()=>{
			//
			// },time_left > 5 ? (time_left - 5)*1000 : 100);
		}
	},
