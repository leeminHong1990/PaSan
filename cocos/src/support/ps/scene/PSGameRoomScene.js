...
var PSGameRoomScene = cc.Scene.extend({
	className: "PSGameRoomScene",
	...
	loadUIManager: function () {
		...
		if(onhookMgr.applyCloseLeftTime > 0 && player && player.curGameRoom){
			if(player.curGameRoom.gameType === const_val.PaSan && player.curGameRoom.playerInfoList[player.serverSitNum]["identify"] == const_ps.GAME_VIEWER){
				//因为是观战者 所以什么都不做
			}else{
