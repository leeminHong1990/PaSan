...
var ClubMemberUI = UIBase.extend({
	...
	initUI: function () {
		...
		var give_btn = left_btn_panel.getChildByName("give_btn");
		var admin_btn = left_btn_panel.getChildByName("admin_btn");
		var black_btn = left_btn_panel.getChildByName("black_btn");

		this.btn_list = [member_btn, apply_btn, search_btn, admin_btn, black_btn, give_btn];
		...
		var give_panel = player_panel.getChildByName("give_panel");
		...
		var page_list = [member_panel, apply_panel, search_panel, admin_panel, black_panel, give_panel];
	...
	init_give_panel: function () {
		var club_player_panel = this.rootUINode.getChildByName("club_player_panel");
		var player_panel = club_player_panel.getChildByName("player_panel");
		var give_panel = player_panel.getChildByName("give_panel");

		var sub_panel = give_panel.getChildByName("sub_panel");
		var info_panel = sub_panel.getChildByName("info_panel");
		info_panel.setVisible(false);

		sub_panel.getChildByName("id_edit_box").setPlaceHolderColor(cc.color(255, 255, 255));
		sub_panel.getChildByName("id_edit_box_give").setPlaceHolderColor(cc.color(255, 255, 255));

		sub_panel.getChildByName("search_mem_btn").addTouchEventListener(function (sender, eventType) {
			if (eventType === ccui.Widget.TOUCH_ENDED) {
				var id_str = sub_panel.getChildByName("id_edit_box").getString();
				if (isNaN(Number(id_str))) {
					if (h1global.globalUIMgr.info_ui) {
		...
		sub_panel.getChildByName("agent_name_label").setString("代理:" + cutil.str_sub(this.club.owner.nickname, 7));
		sub_panel.getChildByName("agent_id_label").setString("ID:" + this.club.owner.userId);

		var info_dict = eval('(' + cc.sys.localStorage.getItem("INFO_JSON") + ')');
		var player = h1global.player();
		var self = this;
		var agent_card_label = sub_panel.getChildByName("agent_card_label");
		// 房卡
		if (info_dict["unionid"]) {
			cutil.get_user_info("wx_" + info_dict["unionid"], function (content) {
				if (!cc.sys.isObjectValid(agent_card_label)) {
					return;
				}
				if (content[0] !== '{') {
					return
				...
				self.owner_cardnum = info["card"];
				agent_card_label.setString("房卡数:" + info["card"].toString());
			});
		} else {
			agent_card_label.setString("******");
			cc.log("info_dict[\"unionid\"] is null");
		}
	},

	update_user_info_give_card:function (userInfo,user_club_list) {
		if(!this.is_show || !this.club){
			return;
		}
		if(!this.club.is_owner(h1global.player().userId)){
		...
		if (this.now_options !== 5) {
			return;
		}
		cc.log(userInfo,user_club_list,this.club.club_id);
		var is_member = false;
		for(var k in user_club_list){
			if(user_club_list[k] == this.club.club_id){
				is_member = true;
		...
		var sub_panel = give_panel.getChildByName("sub_panel");
		sub_panel.getChildByName("Image_10_0").setVisible(true);
		sub_panel.getChildByName("comfirm_give_btn").setVisible(true);
		sub_panel.getChildByName("id_edit_box_give").setVisible(true);
		var info_panel = sub_panel.getChildByName("info_panel");
		info_panel.setVisible(true);
		...
		var agent_card_label = sub_panel.getChildByName("agent_card_label");
		var player = h1global.player();
		sub_panel.getChildByName("comfirm_give_btn").addTouchEventListener(function(sender, eventType){
			if(eventType === ccui.Widget.TOUCH_ENDED){
				var cardnum_str = sub_panel.getChildByName("id_edit_box_give").getString();
				if (cardnum_str.length === 0) {
					if(h1global.globalUIMgr.info_ui){
						h1global.globalUIMgr.info_ui.show_by_info("输入不能为空！");
					}
					return;
				}
				if(isNaN(Number(cardnum_str))){
					if(h1global.globalUIMgr.info_ui){
						h1global.globalUIMgr.info_ui.show_by_info("输入不合法！");
					}
					return;
				}
				var cardnum = Number(cardnum_str);
				if(cardnum <= 0){
					if(h1global.globalUIMgr.info_ui){
						h1global.globalUIMgr.info_ui.show_by_info("输入不能为负数！");
					}
					return;
				}
				if(cardnum > self.owner_cardnum){
					if(h1global.globalUIMgr.info_ui){
						h1global.globalUIMgr.info_ui.show_by_info("房卡不足！");
					}
					return;
				}
				if (!userId) {
					if(h1global.globalUIMgr.info_ui) {
						h1global.globalUIMgr.info_ui.show_by_info("玩家ID错误！请重新搜索！");
					}
					return;
				}
				if (player.userId === userId) {
					if(h1global.globalUIMgr.info_ui) {
						h1global.globalUIMgr.info_ui.show_by_info("不能转给自己！");
					}
					return;
				}
				cutil.lock_ui();

				cutil.give_card(cardnum, userId, function (content) {
					if (!cc.sys.isObjectValid(agent_card_label)) {
						return;
					}
					if (!content["card"]) {
					...
					self.owner_cardnum = content["card"];
					agent_card_label.setString("房卡数:" + content["card"].toString());
				});

				sub_panel.getChildByName("id_edit_box_give").setString("");
			}
		});
	},
});