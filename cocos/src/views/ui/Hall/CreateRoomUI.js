...
var CreateRoomUI = UIBase.extend({
			...
			if (table_params[create_list[i]]) {          // 用for挨个 绘制每一条的按钮和文字
				...
				switch (parseInt(now_params["type"])) {
					...
					case 3:
						...
						card_img.setPositionX(150);
						// panel.setPositionY(right_panel.height * 0.3); //固定房卡位置
					...
					case 4:
						...
						label.setPositionX(40);
						panel.setPositionY(0); //固定label位置
		...
		if (this.chx) {
			...
			for (var k in now_params["values"]) {
				...
				if (now_params["values"][k]["key"] == "hand_prepare") {
					...
					panel.setPositionX(500);
					panel.setPositionY(right_panel.getChildByName("cost_panel").getPositionY());
	...
	set_btn_ui: function () {
		...
		this.createroom_panel.addChild(create_btn);
		create_btn.setPosition(this.createroom_panel.width * 0.5, 85);
	...
	initCreateInfo: function () {
		...
		var default_info_json = '{"now_create_game_name":"ps"}';
