...
var ClubUI = UIBase.extend({
	...
	update_online_state: function (club_id, idx) {
		...
		event_msg["current_round"] = this.club.club_base_info.table_info_list[idx]["current_round"];
	...
	update_sic_info: function (club_id, event_msg) {
		...
		if ((curItem.table_halfway_join === 1 && event_msg["seat_info"].length > 0 && event_msg["seat_info"].length == player_num) || (curItem.table_halfway_join === 0 && event_msg.current_round > 0)) {
	...
	update_table_cur_round: function (club_id, event_msg) {
		...
		cc.error(table_info_list[idx]);
		cc.error(curItem.table_halfway_join);
		if (table_info_list[idx]["room_state"]) {
			...
			if (curItem.table_halfway_join === 0 && event_msg["current_round"] > 0) {
				curItem.getChildByName("desk_btn").setBright(false);
				curItem.getChildByName("desk_btn").setTouchEnabled(false);
				curItem.getChildByName("join_img").setVisible(false);
				curItem.getChildByName("full_img").setVisible(true);
			}
		} else {
	...
	update_room_params: function (club_id, event_msg) {
		...
		//提示增加房间信息
		var room_params = eval("(" + event_msg["room_params"] + ")");
		if (room_params.halfway_join) {
			curItem.table_halfway_join = room_params.halfway_join;
		}
		var add_str = '';
	...
	update_desk_details: function (view, info, index) {
		...
		view.table_halfway_join = room_params.halfway_join;
		if ((room_params.halfway_join === 1 && info["seat_info"].length > 0 && info["seat_info"].length == player_num) || (room_params.halfway_join === 0 && info.current_round > 0)) {
			desk_btn.setBright(false);
			desk_btn.setTouchEnabled(false);
	...
	add_head_frame: function (table_idx, dex, curItem, player_num, info, w, h) {
		...
		head_layer.setPosition(cc.p(w * const_val.PLAYER_POS[player_num][dex].x, h * const_val.PLAYER_POS[player_num][dex].y + 5));
	}
});