...
//
"use strict";
var PSPlaybackGameRoom = cc.Node.extend({
	...
	_getPokerImgPath: function (card) {
		let rank = ps_rules.get_rank(card);
		let suit = ps_rules.get_suit(card);
		// 大小王
		if (rank > 20) {
			...
			return "Poker/pic_poker_small_" + ps_rules.POKER_COLOR_DICT[suit] + "" + rank + '.png';
		}
	},
	...
	update_player_hand_tiles: function (serverSitNum, tileList) {
		...
		for (var i = 0; i < const_ps.HAND_CARD_NUM; i++) {
	...
	parseCommands: function (op_list) {
		for (var i = 0; i < op_list.length; i++) {
			let obj = op_list[i];
			let aid = obj[0];
			if (aid == const_ps.OP_REDEAL) {
				this.commands = [];
			} else if (aid == const_ps.OP_FIGHT_DEALER || aid == const_ps.OP_BET || aid == const_ps.OP_CONFIRM_DEALER || aid == const_ps.OP_MUL) {
				continue;
			} else {
				let command = {};
				command.aid = aid;
	...
	doNext: function () {
		...
		if (commands.length > 0) {
			...
			let player = h1global.player();
			player.PSPostOperation(command.serverSitNum, command.aid, command.tileList, command.nextServerSitNum);
		...
		if (op_special_record) {
			...
			while (flag) {
				...
				if (re[3] == this.totalCommand - this.commands.length) {
					...
					// player.postPlayerDiscardState(re[1], cutil_ps.DISCARD_FORCE);
