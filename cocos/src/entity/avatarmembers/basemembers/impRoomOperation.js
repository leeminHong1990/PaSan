...
var impRoomOperation = impStatOperation.extend({
	...
	enterRoomSucceed: function (serverSitNum, roomInfo, gameType) {
		cc.log("enterRoomSucceed!",serverSitNum, roomInfo, gameType);
		...
		if(gameType == const_val.PaSan){
			this.curGameRoom = eval('new ' + gameName + 'GameRoomEntity(roomInfo["init_info"]["player_num"],gameType)');
			this.curGameRoom.reconnectRoomData(roomInfo);
		}else{
	...
	enterRoomFailed: function (err) {
        ...
        } else if (err === const_val.ENTER_FAILED_ROOM_HALFWAY_JOIN) {
            h1global.globalUIMgr.info_ui.show_by_info("房间不允许中途加入！", cc.size(300, 200));
        }
	...
	give_control:function(idx){
		cc.log("give_control",idx);
		this.curGameRoom.playerInfoList[idx]["is_control"] = 1;
		// this.cellCall("give_control",idx);
	},

	start:function () {
		cc.log("游戏开始");
		this.cellCall("client_start");
	},

	setAutoFollow:function (state) {
		cc.log("setAutoFollow",state);
		this.cellCall("setAutoFollow",state);
	},
