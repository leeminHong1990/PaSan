...
gameroomUIMgr.init_table_idx_panel = function(rootUINode){
    ...
    switch (game_type) {
		...
		case const_val.PaSan:
			var panel = rootUINode.getChildByName("round_info_panel");
			panel.getChildByName("table_idx_label").setString(player.curGameRoom.table_idx+"号桌");
			panel.getChildByName("table_idx_label").setVisible(true);
...
/**
 * function set_ps_auto_btn_anime 为爬三自动按钮添加动画
 * @param wintips_btn 按钮的引用
 * @return null;
 */
gameroomUIMgr.set_ps_auto_btn_anime = function(auto_btn){
	if(!auto_btn){
		return;
	}
	var auto_img = auto_btn.getChildByName("auto_img");
	auto_img.setVisible(true);
	auto_img.runAction(cc.RepeatForever.create(cc.Sequence.create(
		cc.rotateTo(0.833,720)
	)))
};