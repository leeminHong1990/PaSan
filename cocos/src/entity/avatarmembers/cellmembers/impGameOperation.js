...
var impGameOperation = impCommunicate.extend({
	...
	/********************************************gxmj*********************************************************/
	setDiscardState: function (state) {
		this.curGameRoom.discardStateList[this.serverSitNum] = state
		this.cellCall("setDiscardState", state)
	},
	...
	doPSOperation: function () {
		cc.log("doPSOperation: ", arguments);
		if (!this.curGameRoom) {
			return;
		}
		this.gameOperationAdapter.doOperation.apply(this.gameOperationAdapter, arguments);
	...
	psPostOperation: function (serverSitNum, aid, data, nextServerSitNum) {
		cc.log("ps postOperation: ", serverSitNum, aid, data, nextServerSitNum);
		if (!this.curGameRoom) {
			return;
		}
		this.gameOperationAdapter.postOperation(serverSitNum, aid, data, nextServerSitNum);
