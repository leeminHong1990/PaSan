var HelpUI = CreateRoomUI.extend({
	ctor: function () {
		this._super();
	...
	init_now_btn2: function () {
		this.init_list = this.info_dict;
		this.use_list = table_create_params[13];
		this.reset_right_panel();
		return true;
	},

	set_touch_enable: function (panel) {
		var node_list = panel.getChildren();
		for (var k in node_list) {
			if (node_list[k].getName().length > 3) {
				node_list[k].setTouchEnabled(false);
			}
		}
	},
