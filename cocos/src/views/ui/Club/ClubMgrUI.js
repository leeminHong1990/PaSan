...
var ClubMgrUI = UIBase.extend({
    ...
    show_by_info:function (club_id, flag) {
    ...
    initUI:function () {
        ...
        if(is_show_mgr_panel && !this.flag){ //原本是 this.club
	        ...
	        var add_three_table_panel = this.rootUINode.getChildByName("club_mgr_panel").getChildByName("add_three_table_panel");
	        if (this.club.is_white_member()) {
		        mgr_panel.getChildByName("add_three_table_btn").setVisible(true);
		        mgr_panel.getChildByName("add_three_table_label").setVisible(true);
		        mgr_panel.getChildByName("add_three_table_label").setString("当前有" + this.club.club_base_info.table_info_list.length + "张桌子");
		        mgr_panel.getChildByName("add_three_table_btn").addTouchEventListener(function (sender, eventType) {
			        if (eventType === ccui.Widget.TOUCH_ENDED) {
				        add_three_table_panel.setVisible(true);
			        }
		        });
		        add_three_table_panel.getChildByName("confirm_btn").addTouchEventListener(function (sender, eventType) {
			        if (eventType === ccui.Widget.TOUCH_ENDED) {
				        h1global.player().clubOperation(const_val.CLUB_OP_ADD_TABLE, self.club.club_id);
				        add_three_table_panel.setVisible(false);
				        self.hide();
			        }
		        });
		        add_three_table_panel.getChildByName("return_btn").addTouchEventListener(function (sender, eventType) {
			        if (eventType === ccui.Widget.TOUCH_ENDED) {
				        add_three_table_panel.setVisible(false);
			        }
		        });
		        ...
		        add_three_table_panel.setVisible(false);
		        mgr_panel.getChildByName("add_three_table_btn").setVisible(false);
		        mgr_panel.getChildByName("add_three_table_label").setVisible(false);
	        }
        }else{
	        ...
	        club_mgr_panel.getChildByName("return_btn").setVisible(false);
	        normal_panel.getChildByName("return_btn").addTouchEventListener(function (sender, eventType) {
		        if(eventType === ccui.Widget.TOUCH_ENDED){
			        self.hide();
		        }
	        });
